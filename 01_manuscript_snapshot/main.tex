...
% Author block removed for double-anonymous review.
...
\begin{abstract}
We define the Asimov Cascade, a systems-safety process in which robot subsystems that each satisfy their local contracts interact through incompatible assumptions or interfaces and produce an unsafe trajectory. We specify four necessary conditions (C1--C4) that distinguish it from a single interface fault. Because public incident reports rarely expose the chain from perception to actuation, we use patents as a longitudinal, design-disclosing observatory and build an auditable, evidence-gated pipeline that nominates candidate cascade mechanisms for human review. From 9,710 retrieved records, 8,928 publications yielded 77 non-noise topics and 20 screened candidates, five ranked publication-level HIGH. Family normalization over 6,602 simple families with the series censored at 2024 (18-month publication-lag rule) contracted this set to Topics 7, 25, and 40. Only Topic 25 retains its tier under every recent-year perturbation. Independently of the decline score, the held-out family-level strict gate finds Topics 22 and 25 passing its prespecified criterion, and this gate, with the human audit, anchors mechanism nomination. A blinded structured content audit found Topic 22 enriched for force-aware human--robot interaction safety mechanisms. A preregistered one-dimensional controlled simulation pilot produced ordered, repeatable safety-margin erosion under bounded interface mismatch while all modeled local contracts held. Our claims are bounded: patents disclose designs rather than deployment or harm; the prioritization score is review triage, not a risk estimate; and the inverse ASIMOV direction and null topic-level association do not establish a technology-to-safety link. Standards ratings cover 6 of 18 prespecified pairs; runtime or near-miss traces were not collected. The contribution is a conceptual systems model, an auditable monitoring pipeline, and a nominated mechanism supported by a blinded content audit and a controlled model-internal pilot.
\end{abstract}

{\small\par\noindent\textbf{\textit{Keywords}}---\,\ignorespaces
humanoid robots, robot safety, patent analytics, BERTopic, structural topic model, technology management, safety standards\par}

\section{INTRODUCTION}

% \subsection{When Locally Correct Subsystems Compose Incorrectly}
Robot safety has moved over four decades from fencing machines away from people to coordinating machines among them, and among its hardest parts is anticipating how the pieces interact. A combined system can be unsafe even when every subsystem is correct on its own. Asimov's First Law of Robotics states that a robot may not injure a human being or, through inaction, allow a human being to come to harm~\cite{asimov1950}. Behind this fictional ethical principle sits a concrete systems problem, namely that local correctness does not guarantee safe composition. Modern robot stacks decompose this problem into perception, which maps sensor streams to a scene estimate, planning, which selects actions against an objective, and control, which tracks the planned trajectory. A perception module may report correctly under its calibration assumptions, a planner may optimize its objective, and a controller may track the resulting trajectory, but the assembled robot can still be unsafe because those assumptions are mutually incompatible. We call this cross-subsystem process, in which locally correct subsystems produce an unsafe trajectory through incompatible assumptions or interfaces, the Asimov Cascade. It builds on system-theoretic treatments of interaction hazards as system properties~\cite{leveson2011,gupta2024,chakraborty2025,desai2019}. AutoRT pairs semantic constraints from a language-model constitution with conventional environmental controls because such constraints alone provide no safety guarantee~\cite{autort2024,deepmind2024}.
...
This distinction leads to two claims. Claim 1: patents may identify mechanisms that function as safety barriers or propagation nodes (evaluated by the human evidence audit, Section V-A, Table~\ref{tab:review}). Claim 2: temporal and lifecycle patterns may prioritize mechanisms for examination (instrumented by the prioritization score, Section IV-D, and stress-tested by the held-out STM gates, Section V-C, with the gate summary in Table~\ref{tab:layers}). We evaluate Claim 1 and develop an exploratory instrument for Claim 2. Whether a prioritized mechanism predicts a real cascade or human injury lies outside the evidence available in this study. Throughout, the framework detects patent-derived evidence signals rather than incidents, failures, or injuries.

\subsection{Research Questions and Contributions}

We ask which patent topics contain traceable safety evidence (RQ1), which mechanisms an explicit score prioritizes for review (RQ2), whether candidates survive author review, held-out STM tests, and blinded content audit (RQ3), what the ASIMOV stress test shows about the technology-to-safety link (RQ4), and whether a controlled simulation exhibits ordered safety-margin erosion while all modeled local contracts hold (RQ5). Section V answers RQ1--RQ4 in order from Sections V-A to V-D, with the content audit (Section V-E) completing RQ3 and the simulation pilot (Section V-F) addressing RQ5.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.55\textwidth,trim={0 13 0 17},clip]{fig01-doorway-hybrid-v4.pdf}
\caption{Hypothetical mobile-manipulator Asimov Cascade. C1: each module meets its local specification. C2: a base-only path guarantee is consumed as a whole-robot guarantee. C3: the scope mismatch propagates through task execution, trajectory generation, and base control. C4: the base clears the doorway, but the elbow-swept footprint intersects the jamb. The contact inset is a schematic top view, not a magnification or force measurement; the columns are defining conditions, not time steps. This is not a tested implementation or empirical evidence.}
\label{fig:concept}
\end{figure*}
...
where $U(z_{T})$ denotes an unsafe system trajectory. Equation~\eqref{eq:cascade} formalizes the locally checkable conditions C1, C2, and C4; C3 is operationalized by requiring $P$ to contain at least two successive interfaces, so that the deviation must cross at least one downstream subsystem. We label the necessary conditions for later reference: (C1) every subsystem on the path satisfies its local contract; (C2) at least one interface on the path is incompatible; (C3) the incompatibility-induced state or context deviation propagates through at least one downstream subsystem; and (C4) the system reaches an unsafe trajectory. C1, C2, and C4 alone characterize an unsafe compositional event but are not sufficient for a cascade. Without the propagation required by C3, a single incompatible interface is an interface failure, not an Asimov Cascade. Figure~\ref{fig:concept} instantiates this distinction on a hypothetical mobile manipulator passing a doorway. If the task planner discards the stale base-clearance estimate and re-plans from fresh state, the deviation dies at the interface (C1 and C2 hold; C3 does not). In the depicted case, the base-only path guarantee is consumed as a whole-robot guarantee, the mismatch propagates through task execution, trajectory generation, and base control, and the elbow's swept footprint intersects the jamb, so C1 through C4 all hold. Actual injury is a separate event $H(z_{T}) = 1$ that additionally requires human exposure; a near miss may therefore satisfy $\mathcal{AC}(P) = 1$ while $H(z_{T}) = 0$.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.92\textwidth]{fig02-original-style-v4.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.78\columnwidth]{fig03_attrition.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.80\columnwidth]{fig07_kimi.pdf}
\caption{DeepSeek--Kimi cross-model sensitivity: 76/77 valid, 90.8\% agreement ($\kappa = 0.750$), 15 shared candidates, 7 disagreements; Kimi is not an independent reference label.}
\label{fig:kimi}
\end{figure}
...
Documents were embedded with \texttt{AAUBS/PatentSBERTa\_V2}, reduced by cosine-UMAP (seed 42; \texttt{n\_neighbors}=15), and clustered by HDBSCAN (min\_cluster\_size=25)~\cite{grootendorst2022,bekamiri2021,mcinnes2017,mcinnes2018}. Labels used class-based TF-IDF; noise documents were excluded from trend models. Four additional UMAP seeds yielded 75--83 topics, 31.7\%--34.3\% noise, and 14--16 of 20 candidates recovered at document-set Jaccard $\geq 0.5$; parameter changes produced 43--152 topics with 10--12 candidates, and \texttt{all-MiniLM-L6-v2} recovered only 2, so claims are conditional on the reported granularity and embedding.

\subsection{Safety Screening and Evidence Review}

The frozen screen combined a keyword taxonomy with \texttt{deepseek-v4-flash} judgments over topic labels, keywords, and representative abstracts. DeepSeek was chosen for reliable structured JSON output at corpus scale, a practical choice rather than demonstrated superiority; historical calls used a rolling alias at temperature 0.1 and are not retrospectively relabeled as DeepSeek-V4-Flash. The prompt returned a 0--5 score, category, confidence, and rationale; score $\geq 2$ formed the 20-topic review queue. Authors then reviewed patent excerpts and identifiers, assigning safety, harm-link, cascade-role, scope, and confidence labels. To test dependence on a single stochastic draw, five independent re-screenings per model (DeepSeek and Kimi) were run over all 77 topics with the identical frozen prompt; the frozen historical output remains the primary record, reruns serving only as sensitivity analysis. The Kimi endpoint forced temperature 1.0 whereas DeepSeek ran at 0.1; this asymmetry is reported as a confound.
...
One registered deviation: trial duration was extended from 4.0 s to 8.0 s under a preregistered calibration rule (reason and re-hash logged in the simulation audit report).
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.78\textwidth]{fig05_audit.pdf}
...
Topics 65, 74, and 76 illustrate mechanism content spanning propagation and anti-pinch or over-force barrier evidence; the patents report no injury occurrence.

Across five re-screenings per model, DeepSeek nominated 17--18 candidates per run (mean pairwise Jaccard 0.956), reproduced 17 of the 20 frozen candidates in at least four of five runs (a heuristic stability label), and never nominated a new topic; Topics 1, 28, and 65 were the only run-sensitive candidates, and all five raw-HIGH topics were selected in every run. Kimi nominated 11--17 candidates (within-model Jaccard 0.682); paired cross-model runs agreed on 90.6\% of binary labels (mean $\kappa = 0.715$), consistent with the frozen single-run comparison, and neither model is a reference label.

\subsection{Family Normalization and Right-Censoring Reshape the HIGH Set}

The score produced 5 HIGH, 4 MEDIUM, and 11 LOW candidates (Figure~\ref{fig:gates}); under the adjudicated labels, the HIGH set comprises two direct-safety topics (22 and 25), one partial-safety topic (40), and two topics adjudicated Not-Safety (7 and 29). Topics 25 and 40 are cross-platform comparators, and Topic~22 has the clearest force evidence. Read against the review's own scope notes, only Topic~7 was judged out of scope, clustering vehicle lane-changing and human-like driving patents; Topic~29 mixes display, reception, and service robots whose safety devices are incidental to the inventions, so under a humanoid-strict reading it too would fall outside scope (Table~\ref{tab:layers}). Two further raw-HIGH topics, 22 and 25, center on generic collaborative and industrial robots rather than humanoid platforms, a breadth traceable to the retrieval query. Its robot clause OR-combined humanoid and legged terms with the broader human--robot phrase, admitting generic cobot content (Section IV-A). We therefore report scope status per topic for the frozen corpus. A future retrieval can impose a humanoid/legged must-have term or a CPC anchor rather than silently tightening the analyzed corpus.

\begin{figure}[!t]
\centering
\includegraphics[width=0.92\columnwidth]{fig06_gates.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.60\columnwidth]{fig08_ranks.pdf}
...
The frozen publication-level pooled models exactly reproduced the reported inverse coefficients: at k=50, DeepSeek gave $\beta=-3.818$ (p = .0007) and Kimi majority voting $\beta=-2.263$ (p = .1005); the k=25 and k=100 sensitivities were likewise negative for both models (DeepSeek $-3.753$ (p = .0013) / $-3.835$ (p = .0036); Kimi $-2.114$ (p = .160) / $-2.223$ (p = .181)). Reproduction meant agreement within $10^{-10}$ for exposures and $10^{-9}$ for coefficients. The topic-level association remained null ($\rho = -.231$, permutation p = .3669).

In the post hoc k=50 role model, DeepSeek barrier exposure was inversely associated with error in the publication pool ($\beta=-.317$ per SD, 95\% CI $[-.482,-.153]$, q = .0009) and family pool ($\beta=-.215$, 95\% CI $[-.381,-.050]$, q = .0216). Propagation exposure was null (publication $\beta=.104$, q = .425; family $\beta=-.030$, q = .730), and no Kimi role term survived BH correction (minimum $q = .16$ across both pools and three k values). DeepSeek barrier signs remained negative across k=25,50,100 in both pools, but omitting Topic 22 reversed the family barrier sign (Table~\ref{tab:loto}), and propagation estimates were unstable (Figure~\ref{fig:stress}). \textbf{The ASIMOV result does not validate the candidate-prioritization score.} The inverse association was concentrated in the DeepSeek safety-barrier term, with propagation and Kimi role terms null.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.78\textwidth]{fig09_stress.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.92\columnwidth]{fig11_sim_heatmap.pdf}
...
Generalization is limited by the 70.1\% Chinese-authority share, broad search terms, publication lag, and BERTopic sensitivity to embedding and granularity; the recent-year edge in Figure~\ref{fig:corpus} is the publication-lag artifact that motivates the censored-2024 primary (Section IV-A). Family normalization depends on simple-family identifiers and a single earliest representative, and the publication and family analyses disagree on HIGH membership and strict STM confirmation, so neither is a calibrated risk model.

All 77 topics received independent dual-author adjudication, but the label task is intrinsically subjective (Section V-A), and the full-corpus screen is a coarse triage filter rather than a precise classifier. The role-separated ASIMOV analysis was post hoc, reused author-assigned roles, and had strongly unequal neighbor mass (barrier 88.2\% versus propagation 11.6\%; the residual 0.2\% lies outside these two roles); its family barrier estimate also depended on Topic 22. Kimi agreement measures model sensitivity rather than human reliability, and confounds model identity with decoding temperature. Short annual series, filing-based S-curves, heuristic thresholds, fixed STM K=12, and cross-domain ASIMOV errors further limit inference~\cite{gao2013,lee2016}.
...
The content audit is a single-reviewer instrument. The 18-record same-session repeat coding measures repeat agreement, not inter-rater reliability. Topic 22 is neither humanoid-specific nor propagation-explicit (Section V-E), so the audit supports mechanism enrichment rather than a verified collision-protection or cascade category.

The simulation is a model-internal probe only. Its non-monotone delay response forbids reading greater delay as greater propagation, the \textsc{Rescue} result is a matched conservative rescue within this construct rather than a general fix, and the 0 N synthetic contact force precludes any injury interpretation (Section V-F).

\section{CONCLUSION}

This paper defined the Asimov Cascade and proposed an evidence-gated technology-management method for patent monitoring when subsystem-level incident data are not public. Under the censored-2024 family primary, the HIGH set contracts from five publication-level topics to Topics 7, 25, and 40, and only Topic 25 keeps its tier under every recent-year perturbation. Mechanism nomination rests on the strict held-out STM gate, which finds Topics 22 and 25 passing its prespecified criterion, and on the human audit; exact reproduction of the frozen ASIMOV result retained the inverse, barrier-concentrated direction. The blinded content audit identifies Topic 22's enrichment for force-aware human--robot interaction safety mechanisms, while the preregistered one-dimensional pilot demonstrates ordered safety-margin erosion under bounded interface mismatch with modeled local contracts satisfied. Together, these results support mechanism monitoring and model-internal plausibility, not real-world cascade or injury prediction. Generalization beyond patents requires completed standards mappings (6 of 18 pairs coded), runtime or near-miss traces, and richer world models.
...
\else
[Funding statement removed for double-anonymous review.]
\fi

Automated topic-screening judgments were generated by DeepSeek through a rolling model alias at temperature 0.1 (Section~IV-C). Kimi served only for cross-model sensitivity. OpenAI Codex assisted language editing, code review, and vector-layout drafting for Figures~\ref{fig:concept} and~\ref{fig:pipeline}; the authors reviewed all judgments, code, numbers, text, and figures and remain responsible for the analysis. Prompts, settings, seeds, and records are archived with the reproducibility package.

\scriptsize\linespread{0.70}\selectfont
\setlength{\itemsep}{-1.2pt}\setlength{\parsep}{0pt}
...
\bibitem{iso10218_1}
International Organization for Standardization, ``ISO 10218-1:2025, Robotics---Safety requirements---Part 1: Industrial robots,'' Geneva, Switzerland, 2025.

\bibitem{iso10218_2}
International Organization for Standardization, ``ISO 10218-2:2025, Robotics---Safety requirements---Part 2: Industrial robot applications and robot cells,'' Geneva, Switzerland, 2025.
...
International Organization for Standardization, ``ISO/FDIS 13482, Robotics---Safety requirements for service robots,'' final draft international standard, under development, 2026.
...
\bibitem{iso25785_1}
International Organization for Standardization, ``ISO/CD 25785-1, Robotics---Safety requirements for dynamically stable industrial mobile robots---Part 1,'' committee draft, under development, 2026.
